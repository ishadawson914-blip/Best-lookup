...
# Suffix mapping to help OCR match your database
SUFFIX_MAP = {
...
if option == "Street Address" and OCR_AVAILABLE:
    if not CROPPER_AVAILABLE:
        st.error("streamlit-cropper library missing. Check requirements.txt")
    else:
        with st.expander("📸 Scan Address Label", expanded=True):
            ...
            if img_file:
                ...
                st.info("💡 Move the box to cover ONLY the destination address.")
                ...
                if st.button("🚀 Process Scanned Address"):
                    ...
                    with st.spinner("Analyzing text..."):
                        # 1. Enhance Image (Grayscale + Contrast)
                        ...
                        if score > 60:
                            ...
                            st.error("Could not recognize street. Try a tighter crop.")
...
# --- Results Table & Big Card ---
st.divider()

if not results_df.empty:
    # Big Result Card (Fixing the line 163 typo)
    if searched_no:
