...
# Helper to normalize common address abbreviations
SUFFIX_MAP = {
...
# --- OCR & Cropping Section ---
if option == "Street Address" and OCR_AVAILABLE:
    if not CROPPER_AVAILABLE:
        st.error("⚠️ Library 'streamlit-cropper' not found. Please ensure requirements.txt is uploaded and reboot the app.")
    else:
        with st.expander("📸 Scan Address Label", expanded=True):
            ...
            if img_file:
                ...
                st.info("💡 Drag the red box to cover ONLY the address text for better accuracy.")
                ...
                if st.button("🚀 Process Scanned Address"):
                    ...
                    with st.spinner("Analyzing text..."):
                        ...
                        if debug_mode:
                            st.image(enhanced, caption="AI Enhanced View")
                        ...
                        if score > 60:
                            ...
                            if street_box_idx != -1:
                                ...
                                if found_numbers:
                                    # Filter out postcodes (usually starting with 2)
                            ...
                            st.error("Could not match street. Please try a clearer or tighter crop.")
...
if not results_df.empty:
    ...
    st.warning("No sorting record found for this address.")
